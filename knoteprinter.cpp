...
void KNotePrinter::doPrint( const QString &htmlText, const QString &dialogCaption ) const
{
    ...
    for ( int page = 1; page <= textDoc.pageCount(); page++ )
    {
        ...
        painter.drawText(
                ...
                clip.bottom() + painter.fontMetrics().ascent() + 5, QString::number( page )
                );
...
void KNotePrinter::printNote( const QString &name, const QString &htmlText ) const
{
...
void KNotePrinter::printNotes( const QList<KCal::Journal*>& journals ) const
{
    ...
    while ( it.hasNext() ) {
        KCal::Journal *j = it.next();
        header.setHtml( "<html><body><h2>" + j->summary()
                        + "</h2></body></html>" );
